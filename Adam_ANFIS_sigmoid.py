...
class ANFIS:
    def __init__(self, fuzzy_set, rule_number):
        ...
        for j in range(self.fuzzy_set):  # i代表着第A、B个模糊集
            self.fuzzy_a.append([random.random() * 0.5 - 1 for i in range(rule_number)])  # -0.5 ~ 0.5
            self.fuzzy_b.append([random.random() * 0.5 - 1 for i in range(rule_number)])  # -0.5 ~ 0.5
        # subsequence[0],subsequence[ANN_自适应PSO——Diagnosis_iid_2noY_14,34_5_15]...模糊规则后件的参数集合，控制第四层的计算px+qy+r的参数
        self.subsequence = []
        for j in range(self.fuzzy_set+1):
            self.subsequence.append([random.random() * 0.5 - 1 for i in range(rule_number)])  # -0.5 ~ 0.5
        self.MSEs = []
        self.RMSEs = []
        self.MAEs = []
        self.R_squareds = []
    ...
    def output(self, input, train=False):  # 坐标点的x，坐标点的y，两层规则前件，两层规则后件
        mi_fuzzy = []  # 存放所有模糊集下规则的隶属度
        for j in range(self.fuzzy_set):  # j就是第第A、B个模糊集
            mi_fuzzy.append([])
        antecedent = []  # 记录规则适应度
        z = []  # 记录第四层的输出
    ...
    def train(self, inputs, labels, eta, batch_size, epochs=1000, print_error=100):
        ...
        dfuzzy_b = []
        for j in range(self.fuzzy_set):  # i代表着第A、B个模糊集
            dfuzzy_a.append([0 for i in range(self.rule_number)])
            dfuzzy_b.append([0 for i in range(self.rule_number)])
        # （*）记录当次迭代的后件参数
        d_subsequence = []
        for j in range(self.fuzzy_set + 1):
            d_subsequence.append([0 for i in range(self.rule_number)])

        m_fuzzy_a = np.zeros((self.fuzzy_set, self.rule_number))
        v_fuzzy_a = np.zeros((self.fuzzy_set, self.rule_number))
        m_fuzzy_b = np.zeros((self.fuzzy_set, self.rule_number))
        v_fuzzy_b = np.zeros((self.fuzzy_set, self.rule_number))
        m_subsequence = np.zeros((self.fuzzy_set + 1, self.rule_number))
        v_subsequence = np.zeros((self.fuzzy_set + 1, self.rule_number))
        beta1 = 0.9
        beta2 = 0.999
        epsilon = 1e-8
        t = 0
            ...
            for k in range(len(inputs)):
                ...
                if (k + 1) % batch_size == 0 or k == len(inputs) - 1:
                    for i in range(self.rule_number):
                        # bp的反向传播更新前件和后件参数
                        for j in range(self.fuzzy_set):  # i代表着第A、B个模糊集
                            m_fuzzy_a[j][i] = beta1 * m_fuzzy_a[j][i] + (1 - beta1) * dfuzzy_a[j][i]
                            v_fuzzy_a[j][i] = beta2 * v_fuzzy_a[j][i] + (1 - beta2) * (dfuzzy_a[j][i] ** 2)
                            m_hat = m_fuzzy_a[j][i] / (1 - beta1 ** (t + 1))
                            v_hat = v_fuzzy_a[j][i] / (1 - beta2 ** (t + 1))
                            self.fuzzy_a[j][i] -= eta[0] * m_hat / (np.sqrt(v_hat) + epsilon)

                            m_fuzzy_b[j][i] = beta1 * m_fuzzy_b[j][i] + (1 - beta1) * dfuzzy_b[j][i]
                            v_fuzzy_b[j][i] = beta2 * v_fuzzy_b[j][i] + (1 - beta2) * (dfuzzy_b[j][i] ** 2)
                            m_hat = m_fuzzy_b[j][i] / (1 - beta1 ** (t + 1))
                            v_hat = v_fuzzy_b[j][i] / (1 - beta2 ** (t + 1))
                            self.fuzzy_b[j][i] -= eta[0] * m_hat / (np.sqrt(v_hat) + epsilon)
                        ...
                        for k in range(self.fuzzy_set + 1):
                            ...
                            # 更新后件参数
                            m_subsequence[k][i] = beta1 * m_subsequence[k][i] + (1 - beta1) * d_subsequence[k][i]
                            v_subsequence[k][i] = beta2 * v_subsequence[k][i] + (1 - beta2) * (d_subsequence[k][i] ** 2)
                            m_hat = m_subsequence[k][i] / (1 - beta1 ** (t + 1))
                            v_hat = v_subsequence[k][i] / (1 - beta2 ** (t + 1))
                            self.subsequence[k][i] -= eta[0] * m_hat / (np.sqrt(v_hat) + epsilon)
