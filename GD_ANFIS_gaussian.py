...
class ANFIS:
    def __init__(self, fuzzy_set, rule_number):
        ...
        self.fuzzy_a = []
        self.fuzzy_b = []
        # fuzzy_a,fuzzy_b模糊规则前件的参数集合，控制第一层的计算gaussian的参数，并非第一层方框里的东西，而是方框内所需要的对应参数，比如A_1的sigmoid输出需要A_a[0]和A_b[0]
        for j in range(self.fuzzy_set):  # i代表着第A、B个模糊集
            self.fuzzy_a.append([random.random() * 0.5 - 1 for _ in range(rule_number)])  # -0.5 ~ 0.5 高斯中心
            self.fuzzy_b.append([5 for _ in range(rule_number)])  # >0 高斯宽度
    ...
    def gaussian(self, x, c, s):  # c是高斯中心，s是高斯宽度
    ...
    def output(self, input, train=False):  # 坐标点的x，坐标点的y，两层规则前件，两层规则后件
        ...
        for i in range(self.rule_number):
            for j in range(self.fuzzy_set):
                # A_a[i]和A_b[i]是需要优化的参数，此处用的gaussian，标准ANFIS用的高斯函数，俩参数分别代表高斯函数的中心和宽度
                mi_fuzzy[j].append(self.gaussian(input[j], self.fuzzy_a[j][i], self.fuzzy_b[j][i]))  # 第一层的第j个模糊集结果，即隶属度，指定给定的x满足A_1,A_2,...A_rule_number的程度
    ...
    def train(self, inputs, labels, eta, batch_size, epochs=1000, print_error=1):
        # （*）记录当次迭代的前件参数
        dfuzzy_a = []
        dfuzzy_b = []
        for j in range(self.fuzzy_set):  # i代表着第A、B个模糊集
            ...
            for k in range(len(inputs)):
                ...
                for i in range(self.rule_number):
                    ...
                    for k in range(self.fuzzy_set):  # j代表着第A、B个模糊集
                        dfuzzy_a[k][i] += -(y - o) * \
                            ...
                            temp_1[k][i] * \
                            mi_fuzzy[k][i] * (x[k]-self.fuzzy_a[k][i]) / (self.fuzzy_b[k][i] ** 2)

                        dfuzzy_b[k][i] += -(y - o) * \
                            sum([weights[j] * (z[i] - z[j]) if j != i else 0.0 for j in range(self.rule_number)]) / (weights_sum ** 2) * \
                            temp_1[k][i] * \
                            mi_fuzzy[k][i] * ((x[k]-self.fuzzy_a[k][i]) ** 2) / (self.fuzzy_b[k][i] ** 3)
                        if dfuzzy_b[k][i] < 0.01:  # 确保高斯宽度>0
                            dfuzzy_b[k][i] = 0.01
                ...
                if (k + 1) % batch_size == 0 or k == len(inputs) - 1:
                    for i in range(self.rule_number):
                        ...
                        for k in range(self.fuzzy_set + 1):
                            d_subsequence[k][i] = 0
                    self.fuzzy_b = [[x if x>0 else sys.float_info.epsilon for x in sublist] for sublist in self.fuzzy_b]

            MSE = self.MSE(inputs, labels)
            if epoch >= 2:
                if MSE > self.MSEs[-1]:  # 高斯函数尿性，到极限以后会loss回弹
                    break
...
# anfis.train(x_train, y_train, (0.0001, 0.001), ANN_自适应PSO——Diagnosis_iid_2noY_14,34_5_15, 1000)
